...
import { DryFruits, DryProduct } from './DATA'
import ProductCard from './ProductCard'


const DryList = ({ navigation }) => {
    const [productListData, setProductListData] = useState(DryFruits);
    const [activeCategory, setActiveCategory] = useState('All Category');

    const changeProductListData = (title) => {
        setActiveCategory(title)
        if (title == 'All Category') {
            setProductListData(DryFruits)
        } else if (title == 'Dry Fruits') {
            let filtered_products = DryFruits.filter((data) => {
    ...
    return (
        <ViewContainer>
            ...
            <FlatList
                contentContainerStyle={{ alignItems: "center", width: '100%', margin: 0, padding: 15 }}
                horizontal
                data={DryProduct}
